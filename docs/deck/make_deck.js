...
// ── 2. Why ────────────────────────────────────────────────
{
  const s = pres.addSlide();
  s.background = { color: BG };
  s.addImage({ path: IMG('night'), x: 8.1, y: 0, w: 5.23, h: 7.5 });
  s.addText('Why we built this', {
    x: 0.7, y: 0.55, w: 7.0, h: 0.8, fontSize: 36, bold: true, color: INK,
    fontFace: 'Arial', isTextBox: true, margin: 0,
  });
  s.addText(
    'A leaderboard says one model scores 71% and another 73%. That number tells you nothing about your Tuesday night bug, in your codebase, with your weird test setup.',
    { x: 0.7, y: 1.7, w: 7.0, h: 1.2, fontSize: 17, color: INK,
      fontFace: 'Arial', isTextBox: true, margin: 0 });
  s.addText(
    'The only benchmark that matters looks like your actual job: a real repo, a real open issue, and a test suite that must not break.',
    { x: 0.7, y: 3.05, w: 7.0, h: 1.0, fontSize: 17, color: INK,
      fontFace: 'Arial', isTextBox: true, margin: 0 });
  s.addText(
    'So every fight here is exactly that. Two agents get the same bug in separate sandboxed clones. Nobody grades their own homework: the referee judges only on the original tests from main, an honest code review, and how small the diff is.',
    { x: 0.7, y: 4.3, w: 7.0, h: 1.4, fontSize: 17, color: INK,
      fontFace: 'Arial', isTextBox: true, margin: 0 });
  s.addText('And since watching agents work is usually a spinner, we made it a city instead.', {
    x: 0.7, y: 6.0, w: 7.0, h: 0.9, fontSize: 19, italic: true, color: A,
    fontFace: 'Cambria', isTextBox: true, margin: 0,
  });
}
